...
async function resolveDisplayNames(guild, userIds, mockUsers = {}) {
  const entries = await Promise.all([...new Set(userIds)].map(async (userId) => {
    if (mockUsers[userId]) return [userId, escapeMarkdown(`${mockUsers[userId]} (mock)`)];
...
export async function removeRoleFromAllMembersDetailed(guild, role, fallbackIds = []) {
  ...
  const results = await Promise.all([...memberIds].map(async (memberId) => {
    ...
    try {
      ...
      return { failed: memberId, error: error?.message ?? String(error) };
    }
  }));
  return {
    removed: results.flatMap((result) => result?.removed ? [result.removed] : []),
    failed: results.flatMap((result) => result?.failed ? [{ id: result.failed, error: result.error }] : []),
  };
}

export async function removeRoleFromAllMembers(guild, role, fallbackIds = []) {
  const result = await removeRoleFromAllMembersDetailed(guild, role, fallbackIds);
...
export async function upsertManagedMessage(channel, message, payload) {
  if (message) {
    try {
      return await message.edit(payload);
    } catch (error) {
      // The message can be deleted after fetch but before edit. Treat Discord's
      // Unknown Message response as stale persisted UI state and replace it.
...
async function reconcileOverflowMessages(channel, savedIds, embeds) {
  ...
  for (const [index, chunk] of chunks.entries()) {
    ...
    message = await upsertManagedMessage(channel, message, { embeds: chunk });
    ids.push(message.id);
  }
...
export function createRotationUi(store) {
  async function ensure(guild) {
    ...
    const joinMessage = await upsertManagedMessage(
      joinChannel,
      await getMessage(joinChannel, current.ui.joinMessageId),
      { embeds: [waitingEmbed(guild, current, displayNames)], components: joinComponents(current.waitingOpen) },
    );
    const groupingMessage = await upsertManagedMessage(
      groupingChannel,
      await getMessage(groupingChannel, current.ui.groupingMessageId),
      {
  ...
  async function publishGroups(guild, groups) {
    ...
    const groupingMessage = await upsertManagedMessage(ui.groupingChannel, ui.groupingMessage, {
      content: `Groups generated <t:${Math.floor(Date.now() / 1000)}:R>`,
      embeds: embeds.slice(0, 10),
    });
    if (groupingMessage.id !== ui.groupingMessage.id) {
      await store.update(guild.id, (latest) => { latest.ui.groupingMessageId = groupingMessage.id; });
    }
  }
  ...
  async function clearLeaderRolesDetailed(guild, leaderIds) {
    const { leaderRole } = await ensure(guild);
    return removeRoleFromAllMembersDetailed(guild, leaderRole, leaderIds);
  }
