...
After grouping, each leader can run `/lobby code` to add or replace their squad's lobby code. Generated cards and lobby updates both render from the persisted latest grouping, so authorization and card updates continue to work after temporary Rotation Leader roles are cleared. The public grouping card refreshes immediately, and lobby codes reset when the next rotation is generated or `/reset` is used.
